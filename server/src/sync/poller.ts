...
export async function applySyncPayload(payload: SyncPayload): Promise<{
  ...
  const prRows = payload.prs.map((p) => ({
    ...
    prMergeable: p.mergeable,
    prMergeState: p.merge_state_status,
    prReviewDecision: p.review_decision,
    prChecksConclusion: p.checks_conclusion,
    prChecks: p.checks,
      ...
      await tx
        ...
        .onConflictDoUpdate({
          ...
          set: {
            ...
            prMergeState: row.prMergeState,
            prReviewDecision: row.prReviewDecision,
            prChecksConclusion: row.prChecksConclusion,
